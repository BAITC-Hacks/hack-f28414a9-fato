...
def _extract_tasks(segments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    deadline = re.compile(r"(?:\bдо\b|\bк\b|дедлайн\s*[:—-]?|\bсрок\b\s*[:—-]?|мерзімі\s*[:—-]?)\s*([^,.;\n]{2,35})", re.I)
    tasks = []
...
def demo_recording() -> dict[str, Any]:
    """Deterministic example for a presentation when model weights are unavailable."""
    segments = [
        {"start": 0.0, "end": 3.8, "text": "Добрый день. Обсудим план запуска пилота.", "speaker": "Говорящий 1"},
        {"start": 4.0, "end": 9.5, "text": "Айжан, подготовь обновлённый график запуска до 30 сентября.", "speaker": "Говорящий 1"},
        {"start": 9.8, "end": 15.0, "text": "Нұрлан, келісім жобасын подготовьте до 2 октября.", "speaker": "Говорящий 2"},
        {"start": 15.3, "end": 18.0, "text": "Хорошо, отправим материалы обеим командам.", "speaker": "Говорящий 1"},
    ]
    tasks = _extract_tasks(segments)
    return {
        "filename": "Демонстрационный пример (без аудио)",
        "language": "Русский + казахский (пример)",
        "model": "Демо-режим",
        "summary": "Обсудили запуск пилота. Нужно обновить график запуска до 30 сентября и подготовить проект соглашения до 2 октября.",
        "transcript": segments,
        "tasks": tasks,
        "diarization_status": "Демо: говорящие заданы вручную; диаризация записи не выполнялась.",
        "demo": True,
        "notes": ["Это фиксированный демонстрационный пример, не результат распознавания загруженной записи."],
    }
